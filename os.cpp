...
void saveToFile(const string& textToSave) {
...
int main () {
	...
	system("cls");
     ...
     getline(cin,option);
      ...
      if (option == "1") {
        ...
        // Create a thread to save the text to a file
        thread saveThread(saveToFile, textToSave);
        saveThread.join();
    ...
    if (option == "2") {
        ...
        // Create a thread to view the file content
        thread viewThread(viewFileContent);
        viewThread.join();
